 ...
 */
public class AddPresenterTest extends BaseSubversionPresenterTest {
    ...
    public void setUp() throws Exception {
        ...
        presenter = new AddPresenter(appContext, dtoUnmarshallerFactory, eventBus, notificationManager,
    ...
    public void testAddNothingSelected() throws Exception {
        ...
        // Vitaliy Guliy > hope using the ProjectExplorer part instead SelectionAgent may give us such ability
    }
...
//        when(folderNode.getProject()).thenReturn(project);
        ...
        verify(service).add(eq(PROJECT_PATH), argThat(TestUtils.sameAsList(expectedPaths)), anyString(),
                eq(false),
                eq(true),
                eq(false),
                eq(false), asyncRequestCallbackStatusCaptor.capture());
    }
